...
import { getUserByName, genPassword, getUserByEmail, genToken, storeResetToken, getUserByResetToken, updateNewPassword } from "../helpers.js";
...
router.post('/signup', async (req, res) => {
    ...
    try {
        ...
        if (!confirmPassword) {
            return res.status(400).send({ error: "Re-Enter Password field is required" })
        }
        //validate confirm password pattern
        if (!/^(?=.*?[0-9])(?=.*?[a-z])(?=.*?[A-Z])(?=.*?[#!@%$_]).{8,}$/g.test(confirmPassword)) {
            return res.status(400).send({ error: "Re-Enter Password pattern does not match" })
        }
...
router.post('/forget-password', async (req, res) => {
    ...
    const sendEmail = {
        ...
        html: `<h2>The link for reset your password will expire in 1 hour.<a href='https://65d716ad88750687a3d4249b--taupe-belekoy-48d12a.netlify.app/reset-password/${resetToken}'>https://65d716ad88750687a3d4249b--taupe-belekoy-48d12a.netlify.app/reset-password/${resetToken}</a></h2>`
    };
...
router.post('/reset-password/:token', async (req, res) => {
    ...
    try {
        ...
        if (!/^(?=.*?[0-9])(?=.*?[a-z])(?=.*?[A-Z])(?=.*?[#!@%$_]).{8,}$/g.test(newPassword)) {
            return res.status(400).send({ error: "New password pattern does not match" })
        }
        if (!confirmPassword) {
            return res.status(400).send({ error: "Confirm Password field is required " })
        }
        //check confirmPassword pattern
        if (!/^(?=.*?[0-9])(?=.*?[a-z])(?=.*?[A-Z])(?=.*?[#!@%$_]).{8,}$/g.test(confirmPassword)) {
            return res.status(400).send({ error: "Confirm password pattern does not match" })
        }
